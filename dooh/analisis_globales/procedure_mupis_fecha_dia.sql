CREATE OR REPLACE PROCEDURE `mo-advertising-sta.ADVERTISING_ANALISIS_ADHOC.estimacion_impactos_dooh_fecha_hora_multiproveedor`(
...
create table if not exists `mo-advertising-sta.ADVERTISING_ANALISIS_ADHOC.DETALLE_ANALISIS_AUDIENCIAS_MUPI_MULTIPROVEEDOR_FECHA_HORA`  
(

fecha_ejecucion		DATE	,
CAMPAIGN_STRING		STRING	,
FLAG_MUPI_METRO BOOLEAN,
CAMPAIGN		STRING	,
OLA_CIRCUITO		STRING,	
PROVEEDOR		STRING	,
FRAME_ID		STRING	,
...
CLUSTER BY CAMPAIGN_STRING,PROVEEDOR;

------ BORRAMOS ANTES DE INSERTAR

DELETE FROM `mo-advertising-sta.ADVERTISING_ANALISIS_ADHOC.DETALLE_ANALISIS_AUDIENCIAS_MUPI_MULTIPROVEEDOR_FECHA_HORA` 
WHERE CAMPAIGN_STRING=CAMPAIGN_STR
AND FECHA BETWEEN FECHA_INICIO AND FECHA_FIN 
AND FLAG_MUPI_METRO=FLAG_METRO;


INSERT INTO  `mo-advertising-sta.ADVERTISING_ANALISIS_ADHOC.DETALLE_ANALISIS_AUDIENCIAS_MUPI_MULTIPROVEEDOR_FECHA_HORA` 
...
/*
audiencia as (select distinct concat('34',phone_nm) as phone_nm from `mm-new-business-reporting.ADVERTISING.TABLA_PERFILADOS` 

  where service_type='MOBILE' AND SEGMENT_DS IN ('RESIDENCIAL','SOHO')

  AND (EDAD BETWEEN 25 AND 55 AND  RANGO_EDAD_NORMALIZADO LIKE '%ADULTO%')),
...
DATOS_TPT AS (select        
        ...
        from 
              ...
              DATE(aggregation_date) BETWEEN FECHA_INICIO AND FECHA_FIN -- DATE_TRUNC('2025-10-31', MONTH) AND '2025-10-31' 
              and emplazamiento in (select distinct coalesce(emplazamiento,'nulo')  from datos_final_probabilidad_ponderada_mupi_sector)
              AND timecode!='HOME' /*and msisdn in (select phone_nm from audiencia)*/) TPT
...
select CURRENT_DATE() as fecha_ejecucion,CAMPAIGN_STR,FLAG_METRO,A.*,B.*, c.* except(frame_id) FROM
