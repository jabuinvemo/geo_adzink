...
INSERT INTO  `mo-advertising-sta.ADVERTISING_ANALISIS_ADHOC.DETALLE_ANALISIS_AUDIENCIAS_MUPI_MULTIPROVEEDOR` 
...
select * from
DATOS_MUPIS_TOTAL_INI
where  upper(tipo_ubicacion) NOT like '%SUBWAY%' 
--OR upper(tipo_ubicacion) like '%TRAIN%'
...
mupis_circuito AS ( select 'DEMO_RANGO_EDAD_25_55' AS CAMPAIGN,'MULTIPROVEEDOR' AS OLA_CIRCUITO,PROVEEDOR,FRAME_ID 
...
DATOS_TPT AS (select        
        ...
        from 
              ...
              DATE(aggregation_date) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 30 DAY) AND CURRENT_DATE() -- DATE_TRUNC('2025-10-31', MONTH) AND '2025-10-31' 
...
select CURRENT_DATE(),CAMPAIGN_STR,A.*,B.*, c.* except(frame_id) FROM
...
inner join (select frame_id,tipo_ubicacion, direccion as direccion_mupi, provincia as provincia_mupi,coordenadas_looker_mupi,RESOLucion from datos_ubicaciones_completo
