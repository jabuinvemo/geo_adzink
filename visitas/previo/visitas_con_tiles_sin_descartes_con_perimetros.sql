--------------------------------------------------------------------------------
--- version coan calculo de rango usando echas campaña - 20260528
--------------------------------------------------------------------------------
...
--SET HORA_INICIO_OK=10;
--SET HORA_FIN_OK=19;
--SET LISTA_HORAS_OK=[10,11,12,13,16,17,18,19];
...
SET NOMBRE_POC_UBICACIONES_REPORTING='VOLVO_XC40-AON_ABR2026_1';
--SET NOMBRE_POC='CAIXABANK_LATAM-DOOH_ABR2026';
--SET NOMBRE_POC='LIDL_COMPETENCIA_ABR2026_COMPETENCIA-ALDI';
...
--SET LISTA_UBICACIONES=['CENTROS_EL_CORTE_INGLES'];
SET LISTA_UBICACIONES=['TIENDAS_LIDL'];
--SET LISTA_UBICACIONES=['OFICINAS_CAIXABANK'];--DOOH_ABR2026


with
---------------------------- PERIMETRO UBICACIONES----
--- cambiar like
---------------------------------------------------

coordenadas AS (
  SELECT id_ubicacion, direccion, latitud_direccion, longitud_direccion
  FROM `mm-new-business-reporting.ADVERTISING.COORDENADAS_EMPLAZAMIENTOS_INTERES`
  WHERE id_ubicacion LIKE '%TIENDAS_LIDL%'-- <-------cambiar
),
con_tag AS (
  SELECT
    c.id_ubicacion, c.direccion, c.latitud_direccion, c.longitud_direccion,
    e.geometry,
    'tag_encontrado' AS flag
  FROM coordenadas c
  JOIN `mo-advertising-sta.ADVERTISING_ANALISIS_ADHOC.osm_edificios_espana` e
    ON ST_DWITHIN(e.geometry, ST_GEOGPOINT(c.longitud_direccion, c.latitud_direccion), 200)
  WHERE EXISTS (
    SELECT 1 FROM UNNEST(e.all_tags.list) t
    WHERE LOWER(t.element.value) LIKE '%lidl%'
    --   OR LOWER(t.element.value) LIKE '%hipercor%'
    --   OR LOWER(t.element.value) LIKE '%ingles%' OR LOWER(t.element.value) LIKE '%inglés%'
  )
  QUALIFY ROW_NUMBER() OVER (PARTITION BY c.id_ubicacion ORDER BY ST_AREA(e.geometry) DESC) = 1
),
sin_tag AS (
  SELECT
    c.id_ubicacion, c.direccion, c.latitud_direccion, c.longitud_direccion,
    e.geometry,
    'edificio_cercano' AS flag
  FROM coordenadas c
  JOIN `mo-advertising-sta.ADVERTISING_ANALISIS_ADHOC.osm_edificios_espana` e
    ON ST_DWITHIN(e.geometry, ST_GEOGPOINT(c.longitud_direccion, c.latitud_direccion), 25)
  WHERE c.id_ubicacion NOT IN (SELECT id_ubicacion FROM con_tag)
  AND ST_AREA(e.geometry) < 2000
  QUALIFY ROW_NUMBER() OVER (PARTITION BY c.id_ubicacion ORDER BY ST_AREA(e.geometry) DESC) = 1
),
cuadrado AS (
  SELECT
    id_ubicacion, direccion, latitud_direccion, longitud_direccion,
    ST_BUFFER(ST_GEOGPOINT(longitud_direccion, latitud_direccion), 50 / 2) AS geometry,
    'circulo_generado' AS flag
  FROM coordenadas
  WHERE id_ubicacion NOT IN (SELECT id_ubicacion FROM con_tag)
  AND id_ubicacion NOT IN (SELECT id_ubicacion FROM sin_tag)
),

salida as (
SELECT id_ubicacion, direccion, latitud_direccion, longitud_direccion, ST_GEOGPOINT(longitud_direccion, latitud_direccion) AS PUNTO, 
geometry as geometry_ubicacion, ST_PERIMETER(geometry) AS perimetro_metros, ST_AREA(geometry) AS area_m2, flag
FROM con_tag),


------ FIN UBICACION PERIMETRO
...
---- PERIMETRO
salida
--where id_ubicacion='CENTROS_EL_CORTE_INGLES_18'  -- <-----OQUITAR
...
-- eNCAMBIO DISTANCIA POR INTERSECT
--on st_distance(coordenadas_tile,coordenadas_ubicacion) <=75 
ON ST_INTERSECTS(ST_GEOGFROMTEXT(geometry_text), geometry_ubicacion) 
WHERE geometry_text IS NOT NULL AND geometry_ubicacion IS NOT NULL
),
...
SELECT poc,periodo,--id_ubicacion,direccion,

min(fecha) as fecha_inicial,
max(fecha) as fecha_final,

count(distinct id_ubicacion),
count(distinct msisdn) as telefonos,
count(*) as visitas,
count(distinct fecha) as dias,

sum(PROBABILIDAD_PONDERADA) as probabilidad_visita, 
sum(PROBABILIDAD_PONDERADA)/count(distinct id_ubicacion)/count(distinct fecha) as visitas_promedio,

sum(PROBABILIDAD_PONDERADA_total_sector) as probabilidad_visita_ponderada_sector, 
sum(PROBABILIDAD_PONDERADA_total_sector)/count(distinct id_ubicacion)/count(distinct fecha) as visitas_promedio_ponderada_sector,

sum(probabilidad_total) as probabilidad_total,
sum(probabilidad_total)/count(distinct id_ubicacion)/count(distinct fecha) as visitas_promedio_probabilidad_total,

sum(case when probabilidad_visita_ok>0 then probabilidad_visita_ok else 0 end) as probabilidad_visita_ok,
sum(case when probabilidad_visita_ok>0 then probabilidad_visita_ok else 0 end)/count(distinct id_ubicacion)/count(distinct fecha) as visitas_promedio_probabilidad_ok,

MIN(tiempo_total_ok_dia),max(tiempo_total_ok_dia),min(hora_minima)
FROM
(
select A.*, DIRECCION,CONCAT(CAST(latitud_direccion AS STRING), ", ", CAST(longitud_direccion AS STRING)) as coordenadas_looker_ubicacion,

min(DIA_SEMANA) over (partition by msisdn,A.id_ubicacion) as primer_dia_semana,
max(case when  dia_semana =6 and hora_maxima>14 then 1 else 0 end) over (partition by msisdn,A.id_ubicacion) as sabado_tarde,
 
FROM `mo-advertising-sta.ADVERTISING_ANALISIS_ADHOC.DETALLE_VISITAS_GEO_CAMPAIGN_UBICACION`  A 
INNER JOIN 
`mm-new-business-reporting.ADVERTISING.COORDENADAS_EMPLAZAMIENTOS_INTERES` B

ON A.ID_UBICACION=B.id_ubicacion


where brand_ds is not null

) 

--WHERE DIA_SEMANA BETWEEN 2 AND 6 OR (DIA_SEMANA=7 AND hora_maxima<14)
